...
### 2. Are tests there, and did they run successfully?
...
### 4. Attested descriptors are not modified
...
### 5. The index files are not touched
...
The one exception: when a **new descriptor version replaces an attested descriptor** (see [item 4](#4-attested-descriptors-are-not-modified)), the index entry for the affected deployments has to switch from the old file to the new one — changing those specific entries is safe and expected.
...
### 7. Ask for optional improvements
